...
class BackupServer:
    def __init__(self):
        ...
        self.active_transfers = {}  # Tracks active upload/download threads to prevent race conditions
    ...
    def handle_data_transfer(self, token, filename, file_size, protocol, action):
        ...
        thread = threading.Thread(target=transfer_worker, daemon=True)
        transfer_key = f"{token}_{filename}"
        self.active_transfers[transfer_key] = thread
        thread.start()
        ...
        try:
            while True:
                ...
                if cmd == "AUTH":
                    ...
                    # Ensure the background data transfer thread has finished writing to disk!
                    transfer_key = f"{current_token}_{filename}"
                    if transfer_key in self.active_transfers:
                        self.active_transfers[transfer_key].join()
